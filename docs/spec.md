...
`validate` は各行の列集合が `order` と完全一致することを要求する。`order` に記載のない列を持つ行・列数の不一致・`undef` 値はすべて die。
...
## 変更点（design-concept.md 反映）

現行実装との差分：

| 項目 | 現行実装 | 変更後 |
|---|---|---|
| `group()` の内部実装 | カスタム `_build_group` / `_make_group_item` | `TableTools::group` に委譲 |
| `group()` の子列フィルタリング | `order` に列挙された列のみ子に渡す | キー列以外の全列を子に渡す（TableTools 動作） |
| `group()` の `attrs` なし時 | `order` のみで動作（カスタム実装） | `validate($rows, $order)` で型を自動検出 |
| `use TableTools` | なし | `use TableTools qw(validate group detach attach)` を追加 |
| `_build_group` / `_make_group_item` | 存在する | 削除する |
