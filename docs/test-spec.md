...
`group()` に必須なのは `order` のみ。`attrs` は必須条件ではない。
`order` は `TableTools::validate` に渡す完全な列定義であり、全行のすべての列を過不足なく列挙しなければならない。
`order` に含まれない列を持つ行を追加すると `group()` 確定時に die する。
...
| 38 | group() single-level produces grouped items | キー列が外側に出る / キー列以外の全列が `'@'` 配下に入る / キー列が子から除去される（行の列は order と完全一致） |
...
| 43b | group() dies if row has column not in order | order に含まれない列を持つ行がある場合 die（validate の column count mismatch / unexpected column） |
| 43c | group() confirms with 0 items on empty spool | rows が 0 件の場合、validate/group をスキップして 0 件で確定できる |
